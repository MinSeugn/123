...
// turn on the LED if the distance is between dist_min and dist_max
  if (dist_raw >= 100 && dist_raw < 200) {
    analogWrite(PIN_LED, 255-255*((dist_raw-100)/100));
  }
  else if (dist_raw >= 200 && dist_raw <= 300) {
    analogWrite(PIN_LED, 255*((dist_raw-200)/100));
  }
  else {
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) reading = last_reading; // return 0 when out of range.
  return reading;
