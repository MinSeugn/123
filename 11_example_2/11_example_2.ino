...
float dist_min, dist_max, dist_raw, dist_prev, alpha, dist_ema; // unit: mm
...
// adjust servo position according to the USS read value
  ...
  else if(dist_raw > 180 && dist_raw < 360) myservo.writeMicroseconds(_DUTY_MIN+((_DUTY_MAX-_DUTY_MIN)/180)*(dist_ema-180));
...
float USS_measure(int TRIG, int ECHO)
{
 ...
 // LED
  if (reading < 180 || reading > 360) analogWrite(PIN_LED, 255);
