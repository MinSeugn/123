...
#define EMA_ALPHA 0.7     // EMA 필터 값을 결정하는 ALPHA 값. 작성자가 생각하는 최적값임.
...
//ir_cali
const float coE[] = {0.0000532, -0.0204159, 3.7278572, -78.2311434};
...
//    Serial.println(error_curr - error_prev); 
...
float filtered_ir_distance(void){ // 아래로 떨어지는 형태의 스파이크를 제거 후, 위로 치솟는 스파이크를 제거하고 EMA필터를 적용함.
  ...
  dist_raw = coE[0] * pow(dist_ema, 3) + coE[1] * pow(dist_ema, 2) + coE[2] * dist_ema + coE[3];
